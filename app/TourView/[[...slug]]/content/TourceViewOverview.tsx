...
export default function TourceViewOverview(
  //data: IExperienceceViewOverview
  {
    // experience,
    tourDetails
  }:
  ...
  console.log("tourDetails inside:", tourDetails);
  ...
  return <>
    <div className="bg-white rounded-2xl p-8 shadow-lg">
      ...
      <div className="text-gray-600 text-lg leading-relaxed mb-6" dangerouslySetInnerHTML={{ __html: tourDetails.tour.description as string }} />
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        <div className="flex items-center">
          ...
          <div>
            ...
            <div className="text-gray-600">{tourDetails.tour.duration_label}</div>
          </div>
        </div>
        <div className="flex items-center">
          <Users className="w-6 h-6 text-blue-600 mr-3" />
          <div>
            ...
            <div className="text-gray-600">{
              clientType === "booking" ? "-" : groupSizeOnEditor()
            }</div>
          </div>
        </div>
        <div className="flex items-center">
      ...
      {tourDetails.tour.categories && tourDetails.tour.categories.length > 0 && (
        <div className="mb-6">
          <h3 className="text-xl font-bold text-gray-800 mb-3">Categories</h3>
          <div className="flex flex-wrap gap-2">
            {
              // experience.activityCategories.map((category, index) => (
              tourDetails.tour.categories.map((category, index) => (
                <Link
                  href={`/Tours/${category}`}
                  key={index}
