'use client'

import { Lightbox, ZLBImage } from "@/components/Lightbox/ZLightbox";
import { IExperienceCompleteZ } from "@/interface/Interface"
import { IDBTour } from "@/utils/interface/interfaceDatabase";
import Image from "next/image";
import { useState } from "react";

export interface IExperienceViewGallery {
  experience: IExperienceCompleteZ
}

export default function TourViewGallery(
  // data: IExperienceViewGallery
  {
    // experience,
    tourDetails
  }:
    {
      // experience: IExperienceCompleteZ,
      tourDetails: {
        tour: IDBTour
      }
    }
) {

  const [lightBoxOpen, set_lightBoxOpen] = useState<boolean>(false)
  const __images = (): ZLBImage[] => {
    let images: ZLBImage[] = [];
    tourDetails.tour.photos?.forEach(photoURL => {
      images.push({
        src: photoURL,
        alt: tourDetails.tour.title as string
      })
    });
    return images;
  }

  if (!tourDetails.tour.photos || tourDetails.tour.photos.length === 0) return null;
  return <>
    <div className="bg-white rounded-2xl p-8 shadow-lg">
      <h2 className="text-3xl font-bold text-gray-800 mb-6">Photo Gallery</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {tourDetails.tour.photos?.map((photo, index) => (
          <div key={index} className="aspect-video rounded-lg overflow-hidden" onClick={() => {
            set_lightBoxOpen(true)
          }}>
            <Image
              src={photo}
              alt={`${tourDetails.tour.title}`}
              className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"
              width={300} height={200}
            />
          </div>
        ))}
      </div>
    </div>
    {
      lightBoxOpen && (<Lightbox

        images={__images()} onClose={() => {
          set_lightBoxOpen(false)
        }} />)
    }

  </>
}