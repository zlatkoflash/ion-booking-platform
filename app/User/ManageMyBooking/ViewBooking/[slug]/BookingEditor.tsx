...
const BokunBookingEditor: React.FC = () => {
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto bg-white shadow-2xl rounded-xl border border-gray-100 font-sans">
            <h1 className="text-4xl font-extrabold text-indigo-700 mb-2">
                Bokun Booking Editor
            </h1>
            ...
            <div className="mb-8 p-6 border rounded-lg shadow-md">
                ...
                <div className="grid grid-cols-2 gap-8">
                    ...
                    <div className="flex flex-col">
                        <label className="text-gray-700 font-medium mb-2">Adults ({booking.pricing.adultPrice.toFixed(2)} {booking.currency}/pax)</label>
                        <div className="flex items-center space-x-3">
                            <button
                                onClick={() => handleCountChange('adults', -1)}
                                disabled={counts.adults <= 0 || currentStatus === 'CANCELED'}
                                className="p-3 border rounded-full bg-gray-100 hover:bg-gray-200 disabled:opacity-50 transition"
                            >
                            ...
                            <span className="w-12 text-center text-3xl font-extrabold text-indigo-600">{counts.adults}</span>
                            <button
                                onClick={() => handleCountChange('adults', 1)}
                    ...
                    {/* Children Counter */}
                    <div className="flex flex-col">
                        <label className="text-gray-700 font-medium mb-2">Children ({booking.pricing.childPrice.toFixed(2)} {booking.currency}/pax)</label>
                        <div className="flex items-center space-x-3">
                            <button
                                onClick={() => handleCountChange('children', -1)}
                                disabled={counts.children <= 0 || currentStatus === 'CANCELED'}
                                className="p-3 border rounded-full bg-gray-100 hover:bg-gray-200 disabled:opacity-50 transition"
                            >
                                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 12H4"></path></svg>
                            </button>
                            <span className="w-12 text-center text-3xl font-extrabold text-indigo-600">{counts.children}</span>
                            <button
                                onClick={() => handleCountChange('children', 1)}
                                disabled={currentStatus === 'CANCELED'}
                                className="p-3 border rounded-full bg-gray-100 hover:bg-gray-200 disabled:opacity-50 transition"
                ...
                <button
                    ...
                    disabled={currentStatus === 'CANCELED'}
                    className={`mt-6 w-full py-3 font-bold rounded-lg shadow-lg transition duration-150 
            ${currentStatus === 'CANCELED' ? 'bg-gray-400 text-gray-600 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`
                    }
                >
                    {currentStatus === 'CANCELED' ? 'Booking Canceled' : 'Update Counts (Trigger API Re-price)'}
                </button>
            </div>
            ...
            ${currentStatus === 'CANCELED' ? 'bg-red-400 text-red-900 cursor-not-allowed' : 'bg-red-600 text-white hover:bg-red-700'}`}
                >
                    {currentStatus === 'CANCELED' ? 'BOOKING ALREADY CANCELED' : 'Permanently Cancel Booking'}
                </button>
            </div>
