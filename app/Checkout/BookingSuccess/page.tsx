...
import { GetBookingDetailsByToken, IBokunBookingActivityBooking, IBokunBookingPricingCategory, IBokunBookingPricingCategoryBooking } from "@/utils/bokun";

export default async function CheckoutBookingSuccess(params: { searchParams: Promise<{ [key: string]: any }> }) {

  const paramsFor = await params.searchParams;
  console.log("paramsFor:", paramsFor);
  // console.log('paramsFor', paramsFor);
  // console.log("paramsFor.bookingToken:", paramsForSearch.bookingToken);

  const bookingDetailsForToken = await GetBookingDetailsByToken(paramsFor?.bookingToken as string);
  console.log("bookingDetailsForToken", bookingDetailsForToken);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-4xl mx-auto px-6">
        {/* Back to Home Button */}
        <div className="mb-8">
          <Link href={"/Tours"} className="flex items-center text-gray-600 hover:text-gray-800 transition-colors cursor-pointer">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="text-right">
              ...
              <div className="font-mono text-lg font-semibold text-blue-600">
                WIT-{bookingBokun.confirmationCode}
              </div>
            </div>
          </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">{
                        bookingBokun.activityBookings[0].dateString
                      }</div>
                    </div>
                  </div>
                    ...
                    <div>

                      {
                        bookingBokun.activityBookings.map((bookingActivityPart: IBokunBookingActivityBooking) => {
                          ...
                          return <div key={`booking-category-block-${bookingActivityPart.activity.id}`}>
                            {
                              Object.keys(locCategoriesCount).map((catId: string) => {
                                // const catName = bookingActivityPart.activity.pricingCategoryBookings.find((cat: IBokunBookingPricingCategory) => cat.id === catId)?.;
                                ...
                                return (
                                  <div className="font-semibold text-gray-800" key={`item-count-${bookingActivityPart.activity.id}-${catId}`}>
                                    {countParticipants} {
                                      //countParticipants === 1 ? 'Traveler' : 'Travelers'
                                    }
                                    {
                                      categoryNameFOrParticipians?.bookedTitle
                                    }
                                  </div>
                                )
                              })
              ...
              <div>
                ...
                {/*sessionId && (
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="text-sm text-gray-600 mb-1">Stripe Payment Intent ID</div>
                    <div className="font-mono text-xs text-gray-800 break-all">{paymentDB.stripe_payment_intent_id}</div>
                  </div>
                )*/}
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="text-sm text-gray-600 mb-1">Stripe Payment Intent ID</div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Link href={"/Tours"} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-full transition-colors duration-300 cursor-pointer">
            Browse More Tours
          </Link>
          ...
          <Link href={`/api/booking/booking-confirmation?token=${paramsFor?.bookingToken as string}`} target="_blank" className="border-2 border-gray-300 hover:border-gray-400 text-gray-700 font-semibold py-3 px-8 rounded-full transition-colors duration-300 cursor-pointer">
            Download Confirmation
          </Link>
