...
import { attachPaymentMethodToCustomer, signupUserIfNotExist } from '@/utils/user';
...
const StripePaymentForm: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    // now we need to attach the payment method to the customer and save it for feature payments
    const attachingThePaymentMethod = await attachPaymentMethodToCustomer(userRaw.email, paymentMethod.id);
    console.log("attachingThePaymentMethod:", attachingThePaymentMethod);
    if (attachingThePaymentMethod.data.attached !== true) {
      setError("Payment method not attached to customer");
      setLoading(false);
      set_processBooking({ ...processBooking, paidError: true });
      return;
    }
    ...
    console.log("!!! IMPORTANT INFO, PRICE SHOULD BE GENERATED BY THE BOKUN API NOT SEND FROM HERE !!!");
    const paymentIntent = await StripePaymentIntentForTheBooking(
      attachingThePaymentMethod.data.customerId,
      {
